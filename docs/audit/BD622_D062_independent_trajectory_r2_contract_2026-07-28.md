# BD622 D-062 — independent trajectory r2 contract (completed-catalogue anchors)
...
`scripts/audit/d062_independent_trajectory_r2.py` (frozen in the same
...
`N in {3.00, 3.25, ..., 5.50}`; mutant restart `N0 = 4.00`, span
`[4.00, 4.20]`, FD samples `{4.05, 4.10, 4.15}`; holdout `rtol 3e-7`
...
| T8 | mapped spectrum `f_ind = FD(GRID.interpolate(logit, y_rust))` vs `f_e/f_x`: `sup abs(delta_f)/max(f_rust, 1e-8) <= 2e-3` over all 48 nodes x 3 flavours | expected `~1e-4` class (20x); the 24-event-class imprint `~1.8e-3` is borderline at T8 alone — layered rejection is carried by T2 (anchor class), T7-split (flavour structure), T10 (signs); stated honestly |
