# BD622 D-061 — covariance-metrology contract, fourth issue (r4 reissue)
...
Oracle: `scripts/audit/d061_covariance_metrology_r4_oracle.py` (frozen in
...
## r4 supersession delta (adjudicated, D-060)

This contract is the one-change derivative of the preserved D-060 r3 FAIL
(freeze `17fedc6`, report `e40d80a7`), permitted by the registered
adjudication `A-D060-ADJUDICATION` under the D-054/D-055 precedent. The
single change: **N7X gates only on family members with distinct mu/tau
state rows (SA/SB blocks).** On the P-fixed SS continuity states the
exchange-event observable is degenerate — massless elastic energy
conservation (`y3 + y4 = y_alpha + y_beta`) zeroes the Pauli affinity when
the mu and tau logit rows share one slope, leaving `~1e-34` float64
cancellation noise, so the relative N7X metric there is noise/noise — and
the value is computed and RECORDED NON-GATING with a `gated: false`
marker. Every other family member, threshold, formula, mutant, and
interval tier is behaviourally identical to the frozen r3 oracle
(mechanical diff: docstring/contract-name strings, the one gating
predicate, the `gated` record field). The r3 execution's physics results
(all asymmetric members passing, `1522656/0` M-INT-2 containment, five
mutant kills, bitwise D-055 anchors) stand as the preserved FAIL record's
evidence; this reissue re-executes the full frozen family from scratch.
...
no family member was executed before the r3 freeze; the r3 execution (FAIL preserved) is the only prior family exposure and no threshold or band was refitted from it.
