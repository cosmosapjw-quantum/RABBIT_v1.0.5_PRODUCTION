...
```mermaid
flowchart LR
    C[6-10 brief families] --> N{Canonical novelty and cheap no-go screen}
    N -- FAIL --> A[Archive with falsifier]
    N -- PASS --> F[At most 3 complete paper designs]
    F --> S{Prospective strict-dominance selection}
    S -- zero/tie --> Z[No code; no-go or inconclusive]
    S -- exactly one --> P[One external microprototype]
```
...
```mermaid
flowchart TD
    S[Freeze snapshot, hashes, failures] --> R{Provenance and reflection}
    R -- FAIL --> P0[Preserve raw mismatch and stop]
    R -- PASS --> C[Generate 6-10 operation-level families]
    C --> N{Novelty/equivalence screen}
    N -- disguised old route --> A0[Archive candidate]
    N -- genuinely distinct --> T{Up to 3 complete designs}
    T -- FAIL --> A0
    T -- zero/tie --> Q[No code: no-go or inconclusive]
    T -- unique strict-dominance --> PS[Blind preseal of one contract]
    PS -- non-PASS --> Q
    PS -- PASS --> M[Implement one external microprototype]
    M --> E{Experiment, mutation, targeted formal and cost gates}
    E -- FAIL/ERROR --> A0
    E -- INCONCLUSIVE --> A1[Archive as unresolved; no tuning]
    E -- PASS --> O{Owner: intake review only}
    O -- no --> STOP
    O -- yes --> I[Read-only repository intake and exact diff plan]
    I -- non-PASS --> STOP
    I -- PASS --> O2{Separate owner authority for first code PR}
    O2 -- no --> HOLD[Preserve external PASS; no repository change]
    O2 -- yes --> PRS[Candidate-specific fail-closed PR DAG]
```
