# BD622 D-055 — Covariance-metrology contract, corrected reissue (2026-07-28)

Prospectively frozen before any oracle output, under the D-052 omnibus
grant, stage 2 — corrected reissue after the adjudicated BD622_D054 FAIL
(preserved). Per the D-054 adjudication, exactly two design defects are
corrected and nothing else changes: the degenerate equilibrium state is
replaced by a nondegenerate P-closed split state, and M3 is demoted from
bitwise zero to a measured relative bound with the absolute gap recorded. Object: the accepted comparator `_independent_noqke.py`
...
- S-D (envelope point): scales `(1.0, 0.98, 0.98)`
...
| M3 | Electron-sector lemma (measured): the electron modal mu/tau row gap relative to the electron row scale is `<= 1e-10` on every family state, with the absolute gap recorded (realized D-054 class: `~1e-14` relative) | per state |
...
`scripts/audit/d055_covariance_metrology_oracle.py` (frozen with this
