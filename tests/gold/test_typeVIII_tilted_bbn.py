"""Gold gate: Bianchi Type VIII tilted scalar BBN cell."""
...
def test_typeVIII_tilted_zero_structure_recovers_typeI_tilted_cell():
    from rabbit.jax.run_tilted_bbn import TiltedBBNConfig, run_tilted_bbn

    gold = _gold("tilted_typeVIII_N0_sigma001_v1e7")
    ...
    assert type_viii_zero.v_final == pytest.approx(type_i.v_final, rel=1.0e-14)
...
def test_typeVIII_tilted_mixed_sign_curvature_changes_yields():
    from rabbit.jax.run_tilted_bbn import TiltedBBNConfig, run_tilted_bbn

    baseline_gold = _gold("tilted_typeVIII_N0_sigma001_v1e7")
    gold = _gold("tilted_typeVIII_N1_m1em4_N2_1em4_N3_1em4_sigma001_v1e7")
...
def test_typeVIII_tilted_rejects_same_sign_structure_constants():
    from rabbit.jax.run_tilted_bbn import TiltedBBNConfig

    with pytest.raises(ValueError, match="TYPE_VIII requires mixed-sign"):
        TiltedBBNConfig(
            bianchi_type="TYPE_VIII",
            v0=1.0e-7,
            Sigma_H_plus=0.01,
