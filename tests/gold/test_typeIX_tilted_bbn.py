"""Gold gate: Bianchi Type IX tilted scalar BBN cell."""
...
def test_typeIX_tilted_zero_structure_recovers_typeI_tilted_cell():
    from rabbit.jax.run_tilted_bbn import TiltedBBNConfig, run_tilted_bbn

    gold = _gold("tilted_typeIX_N0_sigma001_v1e7")
    ...
    assert type_ix_zero.v_final == pytest.approx(type_i.v_final, rel=1.0e-14)
    assert type_ix_zero.Yp == pytest.approx(gold["Yp"], rel=1.0e-6)
    assert type_ix_zero.metadata["curvature_K_final"] == 0.0
    assert type_ix_zero.metadata["transport_kappa_final"] == 0.0


def test_typeIX_tilted_same_sign_curvature_changes_yields():
    from rabbit.jax.run_tilted_bbn import TiltedBBNConfig, run_tilted_bbn

    baseline_gold = _gold("tilted_typeIX_N0_sigma001_v1e7")
    gold = _gold("tilted_typeIX_N1_1em4_N2_1em4_N3_1em4_sigma001_v1e7")
...
def test_typeIX_tilted_rejects_mixed_sign_structure_constants():
    from rabbit.jax.run_tilted_bbn import TiltedBBNConfig

    with pytest.raises(ValueError, match="TYPE_IX requires same-sign"):
        TiltedBBNConfig(
            bianchi_type="TYPE_IX",
            v0=1.0e-7,
            Sigma_H_plus=0.01,
