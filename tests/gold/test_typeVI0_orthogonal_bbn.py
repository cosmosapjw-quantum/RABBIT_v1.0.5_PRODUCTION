"""Gold gate: Bianchi Type VI0 orthogonal BBN cell."""
...
def test_typeVI0_orthogonal_zero_structure_recovers_typeI_jax_cell():
    from rabbit.jax.driver_classA import JAXClassAConfig, run_classA_jax

    gold = _gold("classA_typeVI0_N0_sigma003")
...
def test_typeVI0_orthogonal_opposite_sign_curvature_changes_yields():
    from rabbit.jax.driver_classA import JAXClassAConfig, run_classA_jax

    baseline_gold = _gold("classA_typeVI0_N0_sigma003")
    gold = _gold("classA_typeVI0_N2_2em4_N3_m1p2em4_sigma003")
    ...
    assert abs(curved.metadata["N2_final"]) > abs(gold["N2_init"])
    assert abs(curved.metadata["N3_final"]) > abs(gold["N3_init"])
...
def test_typeVI0_orthogonal_rejects_same_sign_structure_constants():
