"""Gold gate: Bianchi Type VII0 tilted scalar BBN cell."""
...
def test_typeVII0_tilted_zero_structure_recovers_typeI_tilted_cell():
    from rabbit.jax.run_tilted_bbn import TiltedBBNConfig, run_tilted_bbn

    gold = _gold("tilted_typeVII0_N0_sigma003_v1e7")
    ...
    assert type_vii0_zero.v_final == pytest.approx(type_i.v_final, rel=1.0e-14)
...
def test_typeVII0_tilted_same_sign_curvature_changes_yields():
    from rabbit.jax.run_tilted_bbn import TiltedBBNConfig, run_tilted_bbn

    baseline_gold = _gold("tilted_typeVII0_N0_sigma003_v1e7")
    gold = _gold("tilted_typeVII0_N2_5e3_N3_3e3_sigma003_v1e7")
    ...
    assert abs(curved.v_final) > abs(baseline.v_final)


def test_typeVII0_tilted_rejects_opposite_sign_structure_constants():
    from rabbit.jax.run_tilted_bbn import TiltedBBNConfig

    with pytest.raises(ValueError, match="TYPE_VII0 requires"):
        TiltedBBNConfig(
            bianchi_type="TYPE_VII0",
            v0=1.0e-7,
            Sigma_H_plus=0.03,
