"""Gold gate: Bianchi Type VI_{-1/9} orthogonal BBN cell."""
...
def test_typeVI_m19_orthogonal_rejects_noncanonical_h_minus_one_ninth_data():
...
def test_typeVI_m19_orthogonal_rejects_same_sign_canonical_pair():
    from rabbit.jax.driver_classB import JAXClassBConfig
...
def test_typeVI_m19_orthogonal_canonical_curvature_and_frame_change_yields():
    from rabbit.jax.driver_classB import JAXClassBConfig, run_classB_jax

    baseline_gold = _gold("classB_typeVI_m19_N0_A1e5_sigma001")
    gold = _gold("classB_typeVI_m19_N2_1em4_N3_mN2over9_A1e5_sigma001")
    ...
    assert curved.metadata["N2_init"] == pytest.approx(gold["N2_init"], rel=1.0e-15)
    assert curved.metadata["N3_init"] == pytest.approx(-curved.metadata["N2_init"] / 9.0, rel=1.0e-15)
    assert curved.metadata["N2_final"] > 0.0
    assert curved.metadata["N3_final"] < 0.0
    assert curved.metadata["N3_final"] == pytest.approx(-curved.metadata["N2_final"] / 9.0, rel=1.0e-12)
