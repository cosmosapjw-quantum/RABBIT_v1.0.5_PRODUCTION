"""BD622 D-055 — covariance-metrology oracle, corrected reissue (M1..M5).

Thresholds and the B_native formula are frozen in
``docs/audit/BD622_D055_covariance_metrology_contract_r2_2026-07-28.md``.

Usage: PYTHONPATH=src python3 scripts/audit/d055_covariance_metrology_oracle.py [--out PATH]
"""
...
FAMILY = {"S-A": (1.01, 0.995, 0.995), "S-D": (1.0, 0.98, 0.98),
          "S-C": (1.02, 0.99, 0.99)}
...
def main(argv):
    ...
    for name, scales in FAMILY.items():
        ...
        electron_scale = max(float(np.max(np.abs(modal_electron[2:6]))), TINY)
        electron_gap_rel = electron_gap / electron_scale
        ...
        ok = (r_weak <= 1e-10 and r_mass <= 1e-10 and electron_gap_rel <= 1e-10
        ...
        states[name] = {
            ...
            "electron_pair_gap_rel": float(electron_gap_rel),
    ...
    family_max = {
        ...
        for key in ("R_weak", "R_mass", "R_native", "B_native", "electron_pair_gap", "electron_pair_gap_rel")
    }
    verdict = "PASS" if all_ok else "FAIL"
    report = {"contract": "BD622_D055_covariance_metrology_contract_r2_2026-07-28",
