"""BD622 D-062 — independent trajectory r2 (completed-catalogue anchors, T1..T12).

Frozen contract: ``docs/audit/BD622_D062_independent_trajectory_r2_contract_2026-07-28.md``
...
Usage: PYTHONPATH=src venv/bin/python scripts/audit/d062_independent_trajectory_r2.py [--out PATH]
...
MUT_N0, MUT_N1 = 4.00, 4.20
MUT_SAMPLES = (4.05, 4.10, 4.15)
...
def analyse_run(solution, stats, c0):
    ...
    spectrum_sup = max(
        float(np.max(np.abs(mapped["e"] - rust_fe) / np.maximum(rust_fe, SPECTRUM_FLOOR))),
        float(np.max(np.abs(mapped["mu"] - rust_fx) / np.maximum(rust_fx, SPECTRUM_FLOOR))),
        float(np.max(np.abs(mapped["tau"] - rust_fx) / np.maximum(rust_fx, SPECTRUM_FLOOR))),
    )
...
def main(argv):
    ...
    report = {
        "contract": "BD622_D062_independent_trajectory_r2_contract_2026-07-28",
