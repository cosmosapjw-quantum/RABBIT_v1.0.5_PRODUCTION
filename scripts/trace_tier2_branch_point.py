...
def phase1_probe(cfg: FullCoupledConfig) -> dict:
    ...
    if cfg.tier >= 2:
        T_nu_e_h = float(y[i_tne])
        T_nu_x_h = float(y[i_tnx])
        T_nu_for_rates = T_nu_e_h
    else:
        T_nu_for_rates = float(mod.T_nu_from_T_gamma_tier1(y[i_tg]))
        T_nu_e_h = T_nu_for_rates
        T_nu_x_h = T_nu_for_rates

    weak_h = mod.compute_live_weak_rates(
        f_nue_h, f_nuebar_h, q_gl,
        float(y[i_tg]), T_nu_for_rates, cfg.tau_n,
        compute_iso_reference=False,
        correction_level=cfg.correction_level,
    )
    ...
    return {
        ...
        "phase1_handoff_T_gamma": float(y[i_tg]),
        ...
        "phase1_steps_external": int(len(sol1.t)),
    }
