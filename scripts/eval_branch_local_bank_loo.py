...
def load_entries(manifests):
    ...
    for mp in manifests:
        ...
        for fp in files:
            ...
            branch = "pos" if sigma_plus >= 0 else "neg"
            for sp in ("nue", "nux"):
...
def main():
    ...
    for i, row in enumerate(entries):
        train = [
            ...
            if j != i and r["species"] == row["species"] and r["branch"] == row["branch"]
        ]
        if len(train) == 0:
            out.append({
                "file": row["file"],
    ...
    print(json.dumps({"n_rows": len(out), "out": str(pp.resolve())}, indent=2))
