...
fn array(value: &Value) -> Vec<f64> {
    value["bits"].as_array().expect("array bits").iter().map(bits).collect()
}
...
fn relative(actual: &[f64], expected: &[f64]) -> f64 {
    ...
    let scale = actual.iter().chain(expected).map(|x| x.abs()).fold(f64::MIN_POSITIVE, f64::max);
    actual.iter().zip(expected).map(|(a, b)| (a - b).abs()).fold(0.0_f64, f64::max) / scale
}
...
fn pair_channel_matches_d080b_and_preserves_energy_and_exact_zero_structure() {
    ...
    assert_eq!(data["d080b_blob"], "78489c43f3046db09d8ba2d96070124ed7b0aa91");
    ...
    for case in data["cases"].as_array().unwrap() {
        ...
        let result: F10PairActionTgammaJvp = assemble_pair_action_tgamma_jvp(
            &grid, &state, 2.0, tg, F10ElectronActionConfig::default(),
        ).unwrap();
        ...
        assert_eq!(result.support, serde_json::from_value::<Vec<bool>>(case["support"].clone()).unwrap());
        assert_eq!(result.corrected, serde_json::from_value::<Vec<bool>>(case["corrected"].clone()).unwrap());
        ...
        assert!(relative(&[result.electromagnetic_energy_transfer], &[bits(&case["qem"])]) <= 1.0e-7);
...
fn pair_tangent_matches_centered_primal_and_has_temperature_independent_branch() {
    ...
    let result: F10PairActionTgammaJvp = assemble_pair_action_tgamma_jvp(&grid, &state, 2.0, tg, config).unwrap();
    ...
    for epsilon in [1.0e-3, 3.0e-4, 1.0e-4] {
        ...
        let centered: Vec<f64> = plus.pair_native.iter().zip(&minus.pair_native)
            .map(|(p, m)| (p - m) / (2.0 * epsilon)).collect();
    ...
    assert!(best <= 2.0e-6);
    let moved: F10PairActionTgammaJvp = assemble_pair_action_tgamma_jvp(&grid, &state, 2.0, 2.3, config).unwrap();
...
fn affinity_derivative_matches_high_precision_and_rejects_invalid_input() {
    ...
    for probe in data["pauli_probes"].as_array().unwrap() {
        ...
        assert!((observed - expected).abs() <= tolerance, "Pauli: {observed:.17e} vs {expected:.17e}");
    }
    assert!(stable_pauli_jvp([0.0; 4], [f64::NAN, 0.0, 0.0, 0.0]).is_err());
    assert!(stable_pauli_jvp([f64::INFINITY, 0.0, 0.0, 0.0], [0.0; 4]).is_err());
    let grid = F10ActionGrid::affine_legendre(8, 8.0).unwrap();
    assert!(assemble_pair_action_tgamma_jvp(&grid, &[], 2.0, 2.0, F10ElectronActionConfig::default()).is_err());
    assert!(assemble_pair_action_tgamma_jvp(&grid, &[0.0; 24], 2.0, 0.0, F10ElectronActionConfig::default()).is_err());
}
