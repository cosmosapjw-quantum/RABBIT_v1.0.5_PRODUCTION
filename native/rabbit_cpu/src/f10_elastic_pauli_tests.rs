...
fn batch(p1: f64, temperature: f64) -> crate::f10_action_kinematics::F10KinematicBatch {
    ...
    two_body_kinematics(F10KinematicInput {
        p1, p2_nodes: &nodes, p2_weights: &weights, mass2: MASS, mass3: 0.0,
        mass4: MASS, config,
    }).unwrap()
}
...
fn relative(actual: &[f64], expected: &[f64]) -> f64 {
    ...
    actual.iter().zip(expected).map(|(a, b)| (a - b).abs()).fold(0.0, f64::max) / scale
}
...
fn elastic_pauli_and_moving_location_match_centered_primal() {
    ...
    for (p1, temperature) in [(2.0, 2.05), (0.75, 0.4)] {
        ...
        for i in 0..base.support.len() {
            ...
            if base.support[i] && y3 > 0.0 && y3 < grid.y_max {
                let value = elastic_pauli_location_tangent(
                    &grid, &target_logits, -0.7, base.e2[i], tangent.d_e2[i],
                    base.e4[i], tangent.d_e4[i], temperature, y3,
                    tangent.d_p3_magnitude[i] / 2.0,
                ).unwrap();
                assert_eq!(
                    value.base.to_bits(),
                    stable_pauli_gain_minus_loss(value.logits).unwrap().to_bits()
                );
        ...
        for factor in [1.0e-3, 3.0e-4, 1.0e-4] {
            ...
            let expected: Vec<f64> = indices.iter().map(|&i| {
...
fn elastic_pauli_location_rejects_invalid_shapes_and_coordinates() {
    let grid = F10ActionGrid::affine_legendre(8, 8.0).unwrap();
    assert!(
        elastic_pauli_location_tangent(
            &grid, &[0.0; 7], 0.0, 1.0, 0.0, 1.0, 0.0, 1.0, 1.0, 0.0
        )
        .is_err()
    );
    assert!(
        elastic_pauli_location_tangent(
            &grid, &[0.0; 8], 0.0, 1.0, 0.0, 1.0, 0.0, 1.0, 9.0, 0.0
        )
        .is_err()
    );
}
